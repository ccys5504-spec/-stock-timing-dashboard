...
2026-09-20: 비교 기준선에 "같은 종목군 동일가중 보유"를 추가했다(코스피 지수만
비교하면 종목군 자체의 생존편향이 전략 성과로 착각됨). 조회에 실패한 종목은
조용히 빠지지 않고 목록으로 출력한다.
...
def main() -> None:
    ...
    for label, start, end in WINDOWS:
        ...
        ew_total, ew_mdd = summarize_equity(
            equal_weight_equity(price_data, pd.Timestamp(start), pd.Timestamp(end))
        )
    ...
    print(f"{'구간':<42} {'누적수익률':>9} {'CAGR':>7} {'MDD':>7} {'샤프':>5} "
          f"{'매매':>5} {'ATR손절':>6} {'코스피보유':>9} {'동일가중보유':>11} {'(MDD)':>7}")
    for r in rows:
        cagr_s = f"{r['CAGR']:+.1%}" if r["CAGR"] is not None else "—"
        sharpe_s = f"{r['샤프']:.2f}" if r["샤프"] is not None else "—"
        bh_s = f"{r['코스피보유']:+.1%}" if r["코스피보유"] is not None else "—"
        print(
            f"{r['구간']:<42} {r['누적수익률']:>+8.1%} {cagr_s:>7} {r['MDD']:>+6.1%} "
            f"{sharpe_s:>5} {r['매매횟수']:>4}회 {r['ATR손절']:>5}회 {bh_s:>9} "
            f"{r['동일가중보유']:>+10.1%} {r['동일가중MDD']:>+6.1%}"
        )
    print(
        "\n※ 동일가중보유는 '오늘 시점 시총 상위 종목'을 과거부터 그대로 들고 있었다는 "
        "가정이라 생존편향이 섞인 상한선입니다(비용 미반영, 실제 투자 가능한 대안 아님)."
    )
